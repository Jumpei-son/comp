...
contains
  ...
  real function turbid_density(t, s)
    ! turbid(mixture) water density, kg/m3
    real, intent(in) :: t ! temperature, deg-C
    real, intent(in) :: s ! suspended solids conc., g/m3
    real :: rho
    rho = water_density(t)
    turbid_density = rho + s/1000*(1. - rho/rho_ss)    
...
module mod_domain
  ...
  type :: domain
    ...
    real, pointer, dimension(:,:) :: &
      ...
      GRAV,   &  ! gravity force from river gradient
    ...
    real ::          &
      ...
      b_in,          &    ! inflow width, m
      z_in_lowlim         ! lower limit of inflow elevation, m
    real, pointer :: &
    ...
    logical, pointer :: &
      trib_pios(:)        ! tributary flow (=T) or not (=F)
    real, pointer :: &
...
contains
  ...
  subroutine deallocate_variables(dom)
    ...
    nullify ( &
      ...
      dom%s_sed)
    nullify (dom%PGX, dom%PGXraw, dom%PGXold)
    nullify (dom%tke, dom%td_eps, dom%nut)
    nullify (dom%exceed_logged)
  ...
  subroutine set_initial(dom)
    ...
    ! init. turbulence variables (k-epsilon)
    dom%tke = 1.25e-7
    dom%td_eps = 1.0e-9
  ...
  subroutine set_turbulence(dom)
    ...
    logical :: freeslip = .false.       ! impose(T) or expose(F) freeslip at surf bot using k-epsilon 
...
contains
  ...
  subroutine set_boundary(dom)
    ...
    namelist /boundary/ &
      id_up, fr_in, b_in, z_in_lowlim, fname_in, &
    ...
    ! default parameters
    id_up=0; fr_in=0.25; b_in=100.; z_in_lowlim=-999.; fname_in=''

    n_out=1; surf_out=.false.; z_out=-999.; fr_out=0.134; phi_out=90.
    fname_out=''
    out_height= 2.0; zKTSW= 999.0; zKBSW=-999.0
    ...
    ! parameter setting
    dom%id_up=id_up; dom%fr_in=fr_in; dom%b_in=b_in; dom%z_in_lowlim=z_in_lowlim
  ...
  !-- SELECT CASE  normal distributed inflow at equidensity level
  CASE ('EDI')        
  ...
  !-- SELECT CASE  normal distributed inflow at river inflow
  CASE ('RID')
  ...
  !-- SELECT CASE  single cell river inflow due to density selection
  CASE ('RI1')
  ...
  subroutine update_outflow(dom, time_day)
    ...
    allocate (f(dom%nz), s_(nps), u(dom%nz), q(dom%nz), rho_outavg(dom%nz), volay(dom%nz))
    !-----------------------------------------------
    ...
    if (dom%n_out > 0) then  ! dam with some outlets
      ...
      !-- SELECT CASE  normal distributed outflow
      CASE ('NDO')       
        do j=1, dom%n_out
      ...
      !-- SELECT CASE  equal distributed in each gate cell
      CASE ('EDO') 
        do j=1, dom%n_out
          kc = -1
          ...
          ! equal distribution outlet velocity, temperature and suspended solids
          u(:) = 0.0
          if(kc_thick > 1) then
            do k= kc_dw, kc_up
      ...
      !-- SELECT CASE   density denpended distributed outflow
      CASE ('DDD')
        ...
        ! each layer average density near dam  
        do k = 1, k_srf
          do i=rhonx, nx
            if(k >= dom%kc_bot(i)) then
              volay(k) = volay(k) + dom%vol(i,k)
              rho_outavg(k) = rho_outavg(k) + dom%vol(i,k)*turbid_density(dom%t(i,k), sum(dom%s(i,k,:)))
            end if
        ...
        do j=1, dom%n_out
          ...
          ! SWITCH user diffinition KTSW and KBSW
          if (ktswkbsw) then
            KTOP = k_srf
            KBOT = kc_bot(nx)
    ...
    !--- de-allocate arrays ---
    deallocate (f, s_, u, q, rho_outavg, volay)
    !--------------------------
...
do i=0, nx+1
  do k=0, nz+1
    do j=1,nps
      if (abs(dom%s(i,k,j)) < 1e-20) dom%s(i,k,j) = 0.0
    enddo
  enddo
enddo
...
contains
  ...
  subroutine set_heat()
    ...
    ! namelist (heat)
    namelist /heat/ radiation, ar, beta, eta, z_wind, coeff_solar, alpha_heat, fname_meteo
    ...
    do i=1, nx
      do k=kc_bot(i), k_srf
        ...
        if (k == kc_bot(i)) then
          ...
          fm = alpha_heat*dhz(i,k-1)*(t(i,k) - t(i,k-1))/dzs(k-1)*aw(i,k-1)
        endif

        if (k == k_srf) then
          fp = 0.0
        else
          fp = alpha_heat*dhz(i,k)*(t(i,k+1) - t(i,k))/dzs(k)*aw(i,k)
        endif
    ...
    do i=1, nx
      do k=kc_bot(i), k_srf
        ...
        if (i == 1 .or. k <= k_bot(i-1)) then
          ...
          fm = alpha_ss*dcx(i-1,k)*(s(i,k,l) - s(i-1,k,l))/dxs(i-1)*au(i-1,k)
        endif

        if (i == nx .or. k <= k_bot(i)) then
          fp = 0.0
        else
          fp = alpha_ss*dcx(i,k)*(s(i+1,k,l) - s(i,k,l))/dxs(i)*au(i,k)
        endif
        ...
        if (k == kc_bot(i)) then
          ...
          fm = alpha_ss*dcz(i,k-1)*(s(i,k,l) - s(i,k-1,l))/dzs(k-1)*aw(i,k-1)
        endif

        if (k == k_srf) then
          fp = 0.0
        else
          fp = alpha_ss*dcz(i,k)*(s(i,k+1,l) - s(i,k,l))/dzs(k)*aw(i,k)
        endif
...
contains
  ...
  real function surface_oxygen_flux(o, t)
    ...
    real, parameter :: &
      ...
      l_f0 = 440.e-7 !440.e-6         ! reference surface film thickness, m
